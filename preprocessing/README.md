...
Use Python 3.12 and install the dependencies in `requirements.txt` in a dedicated environment. Open the notebook in a Jupyter-compatible editor and use a desktop plotting backend for interactive figures.
...
1. Work from this directory. Set `DATA_ROOT` to the downloaded `BIDS_Data` folder and `OUTPUT_ROOT` to a separate results folder.
...
Processing settings: 0.03-50 Hz filtering, a 50 Hz notch with 4 Hz width, 100 Hz sampling, -1.5 to 5 s epochs, a -0.1 to 0 s baseline, ICA fitted on a 1 Hz high-pass-filtered 0-3 s copy, and M1/M2 referencing.

The notebook exports NoReject epochs; trial rejection and stimulus-level ERP averaging are not included.
